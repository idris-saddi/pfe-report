...
| Pattern | Count | Notes |
|---|---|---|
| "X rather than Y" antithesis | **31×** | The single strongest tell. Often several per page ("a hard constraint rather than an afterthought", "defaults rather than premium options", "a present concern … rather than a deferred ambition", "demanded today rather than eventually", "at its seams rather than at its center"…). Keep the load-bearing ones, rewrite the rest as plain statements. |
...
| Punchy verdict fragments | ~10 | "The costs are real." · "The choice is not close." · "Three points decide it." · "There is one backend, not many." · "No single control is trusted to be enough." · "This is the strongest driver." · "Network isolation is by design." · "The system clock is the fifth actor." — blog cadence, not report cadence. Merge into neighbouring sentences. |
| Epigram pairs "X survives; Y does not" | 2× | Ch III L404–405 "A public edge faces out; an internal network does not." and L536–537 "The record that something happened survives; the personal data within it does not." Same rhetorical mould twice = visible template. |
| "surface" as verb/noun-of-art | 10× | "surfacing them", "surfaces the portfolio", "authentication surface", "everyday surface", "owns this surface", "small custom surface"… Product-manager jargon; vary with "show", "display", "expose", "raise". |
| "seam(s)" | 16× | Deliberate structural device (Ch III §8), fine as a *named* concept — but 16 occurrences dilutes it. Keep it for §8 + Ch V bridge, thin it elsewhere. |
| "deliberately / intentionally / on purpose" | 7+ | Authorial defensiveness ("deliberately left", "deliberately not built", "bypasses … deliberately", "intentionally narrow", "absent on purpose"). One or two suffice for the whole report. |
| "posture" | 7× | "compliance posture", "observability posture" — consulting jargon; "compliance position/stance" or just "compliance" usually works. |
| "machinery" | 5× | "invitation machinery", "session machinery", "multi-tenant machinery"… pick "mechanisms"/"components" for some. |
| "forward-looking" | 5× | Stock modifier for the R&D work; already implied by "planned". Cut most. |
| "first-class" (object/aggregate/record) | 4× | CS-idiom fine once; 4× is a tic. |
| "shape / shaped / solution shape" | 7× | "common solution shape", "aggregate-shaped", "The same shape — fetch, extract…". AI-engineerese; say "structure" / "form" / spell it out. |
| Poetic inversion "Above/Beneath/Around X sits Y" | 6× | "Around comby sits a broader engineering identity", "Above them sits a platform-level system administrator", "Beneath them sits the isolation…", "Four stores sit behind them"… Once is elegant; six is a pattern. |
| Personification of artefacts | many | code "remembering", documents "live", text "lives", writes are "careful", refactoring "keeps the design honest", the framework "reaches further than the product uses", a format that "survives the application". |
| Repeated signature phrases | — | "stale the moment it is finished/delivered" **3×**; "entry fee" **3×**; "turns … from the product to the {system,framework} {beneath it,it runs on}" **3×** (Intro L52–54, Ch IV L167–168, Ch V L59–61); "who approved what, when, and on which version" **3×**; "deferred ambition" **2×**; "entry point of/for every other capability" **2×**; "a format that survives the application" **2×**; "where it is meant to grow" **2×**; bolt metaphor ("bolted on top", "bolt-on", "bolting on") **3×**. Each is fine once — the repeats look copy-pasted. |
| Invented compound coinages | — | "overlay-on-incumbents", "DACH-tailored productisation", "aggregate-shaped", "broker-fanned", "designed-for but dark". Hyphenated neologisms are a strong generator tell; unpack them into plain phrases. |
