...
% Renders Chapter2/figures/<name>.{pdf,png} once it exists; until then shows a
...
\newcommand{\pumlfig}[4]{%
  \begin{figure}[!ht]\centering
    \IfFileExists{Chapter2/figures/#1.pdf}{\includegraphics[width=#2]{#1}}{%
    \IfFileExists{Chapter2/figures/#1.png}{\includegraphics[width=#2]{#1}}{%
...
Chapter~I focused on \emph{what} CLMPilot needs to do, and under which constraints; 
this chapter addresses the \emph{how}. It details an architecture 
designed to satisfy those requirements by construction rather than through continuous 
...
handles only their lifecycle management (Section~\ref{sec:competitive-landscape}). 
...
demanded by GoBD, GDPR, and ISO~27001 (Section~\ref{sec:nfr-compliance}). 
...
other~\cite{KurrentEventSourcing}.
...
And -- the biggest saving -- its defaults supply the tenancy, identity, RBAC
...
Chapter~I's requirements decompose into four core \textit{bounded
contexts}, each with its own aggregate, events
...
\pumlfig{identity-model}{0.52\textwidth}{The IAM : identity and access model.}{fig:identity-model}
...
comby~v3 adds a deeper reinforcement -- row-level security in the database
itself, so that the data layer would refuse a cross-tenant read even if the
application layer were wrong -- which CLMPilot does not yet activate.
...
us cover Chapter~I's requirements over a small custom surface. The
