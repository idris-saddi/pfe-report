...
The non-functional requirements become 5 drivers. Each is a
...
\pumlfig{comby-layers}{0.85\textwidth}{Layering of CLMPilot on comby: the 4
core domains, the enterprise contexts that extend them, and 2 cross-cutting
...
\pumlfig{backend-packages}{\textwidth}{Package structure of the backend:
...
Figure~\ref{fig:domains} shows the 4. Contract is central; Approval,
...
\pumlfig{domain-model}{0.85\textwidth}{The 4 bounded contexts and their
...
aggregate.\\Soft delete (tombstone) and restore are available from any state and are omitted
 here for clarity.}{fig:contract-states}
...
(Figure~\ref{fig:topology}). 3 runtime services come from their own
...
itself. The services are backed by 4 stores. \textbf{PostgreSQL} is the system of
...
The 4 repositories separate by concern and release cadence, not by
...
cross-cutting change (like a new API field) touches 2 repositories.
...
on. It also fixes where that structure is meant to grow. 3 seams were left 
open.
...
it sets 3 clear growth points: snapshotting and projection persistence so
