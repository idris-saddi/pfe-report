...
Three primary forces drive the design. \textbf{Integration-first}: contract
...
itself. The services are backed by four stores. \textbf{PostgreSQL} is the system of
...
concrete provider is interchangeable; ERP integration is anticipated in the
design but not yet built.
...
apart, and finally compliance, which proves to be mostly a
...
per-tenant authorisation are the two layers just described. Beneath those two
...
Human-readable text appears in two places, one per process. The single-page
...
single logical event store is partitioned by that context. The framework already provides more than
...
isolation compliance depends on, remains open for now.
...
strategy as the log grows without bound, and the multi-instance topology,
fanned out through the message broker, that Section~\ref{sec:deployment-topology}
already wires for but does not yet run.
...
documents}. The same pattern (fetch, extract, event-source the result, enrich a
