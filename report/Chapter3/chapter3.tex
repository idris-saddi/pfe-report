...
\textbf{Selection criteria.} 4 criteria follow from the drivers. The
first is \emph{compliance-grade history}: the record must be immutable
and attributable, and any past state must be reconstructable on demand;
this is the auditability driver, and the decisive criterion. The second
is \emph{domain fit}: contract management is a read-heavy workload
(dashboards, expiry counts, sums by quarter) punctuated by rare, guarded
writes (an approval, an edit) that must pass multi-step validation. The
third is \emph{tenant isolation by construction}: whatever the storage
model, every record and every access must carry the tenant it belongs to.
The fourth is \emph{maintainability}: the result must stay operable by a
single engineer, without a heavy runtime beside the application.

\textbf{Candidate architectures.} 4 candidate families could carry a CLM
platform. \emph{Classic CRUD}, first, is the default 3-tier
architecture~\cite{Fowler2003}: the current state lives in mutable rows
and every update overwrites the previous one; its usual refinement bolts
an audit trail beside the data (audit columns, history tables, triggers)
to log the changes. \emph{A document-oriented store} models each contract
as a self-contained document with a flexible schema, which suits
heterogeneous metadata. \emph{A workflow (BPM) engine} makes the process
the first-class citizen and models approval chains natively. \emph{CQRS
with Event~Sourcing}, finally, stores what happened rather than what is:
an append-only sequence of events is the primary record, the current
state is derived from it, and reads are served by models kept separate
from the writes~\cite{FowlerEventSourcing,FowlerCQRS,Young2010}.

\textbf{Justification of the choice.} The first criterion eliminates most
of the field. Classic CRUD keeps no history at all; with audit columns
the trail becomes a side-structure that can be forgotten or edited, and
it still cannot reconstruct a past state, exactly the guarantee that the
regulator wants. A document database inherits the same weakness and adds
one of its own: it has no transactional model for a multi-step approval.
A workflow engine would model approvals natively, but it adds a heavy
runtime and vendor lock-in, against the maintainability criterion, and
still leaves the contract's own audit trail unsolved. CQRS with
Event~Sourcing is the only candidate that meets the first criterion by
construction (the history is not kept beside the data, it \emph{is} the
data) while fitting the split read/write workload and carrying a tenant
context naturally on every event. Its price falls on the fourth
criterion: eventual consistency, long-lived event schemas, and a learning
curve. That price is accepted knowingly, and
Section~\ref{sec:cqrs-es-pattern} returns to it: state can always be
re-derived later, but a history that was never recorded cannot be
retrofitted. CQRS and Event~Sourcing were therefore chosen before any
...
\pumlfig{comby-layers}{0.80\textwidth}{Layering of CLMPilot on comby: the 4
...
(Section~\ref{sec:seams}). An \textbf{object store} keeps exports, signed
