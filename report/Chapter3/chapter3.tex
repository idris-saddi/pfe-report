...
\pumlfig{cqrs-es}{\textwidth}{The CQRS and Event Sourcing flow}{fig:cqrs-es}

Figure~\ref{fig:cqrs-seq} shows the same flow in time. The command path
is short and synchronous: validate, append an event, answer the caller.
The reactions come after, once the caller already holds its answer: a
projection updates the read models, and a reactor turns the new event
into a follow-up command. That is the reactor pattern, and the point
where eventual consistency becomes visible.

\pumlfig{cqrs-sequence}{0.8\textwidth}{The command path and its reactions,
in time}{fig:cqrs-seq}
...
% Breakable (xltabular, not a float): the table is taller than the space it
% usually lands in, and as a [H] float it was pushed whole to the next page,
% leaving the page before it half empty. As a longtable it starts under this
% sentence and continues overleaf, with the header row repeated there.
{\footnotesize
  \begin{xltabular}{\linewidth}{|>{\bfseries}l|l|X|X|}
...
% 0.72 rather than 0.45: the diagram is now laid out wide, so it needs more of
% the line to keep its boxes at the size they had when it was narrow.
\pumlfig{component-view}{0.85\textwidth}{CLMPilot component view}{fig:component-view}
...
\pumlfig{topology}{\textwidth}{CLMPilot deployment topology}{fig:topology}
...
\pumlfig{security-pipeline}{0.9\textwidth}{The request security pipeline}
...
workspace-scoped grant beneath the tenant, which CLMPilot does not yet
exercise.

\pumlfig{authz-decision}{\textwidth}{Authorisation process of commands and queries}
{fig:authz-decision}
...
\pumlfig{security-layers}{\textwidth}{Defence in depth}{fig:security-layers}
\vspace{1cm}
