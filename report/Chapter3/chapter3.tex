...
(Section~\ref{sec:topology}), and our approaches to security and internationalisation
...
The alternatives are ruled out by the same driver. CRUD with audit columns makes the
...
here because a framework supplies the mechanics; that framework is the subject
of the next section.
...
the system uses SendGrid for email, the users' own cloud storage for documents,
...
\pumlfig{identity-model}{0.5\textwidth}{The identity and access model.}{fig:identity-model}
...
this is the right trade-off (it keeps operations simple), but
...
applies back to the contract. That pipeline is built around \emph{contract
...
on a multi-repository, single-backend topology hosted in the EU, with tenant isolation
and the audit trail as structural guarantees. Its open seams mark where the
design is built to extend.

Chapter~\ref{chap:realisation} turns from this design to its realisation,
consolidating the work delivered across the project: foundations, the contract
